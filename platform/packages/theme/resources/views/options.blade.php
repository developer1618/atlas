        ...
        <div class="theme-option-sticky">
            <div class="info_bar">
                <div class="theme-option-action_bar">
                    ...
                    <button class="btn btn-primary button-save-theme-options"
                        type="submit">{{ trans('packages/theme::theme.save_changes') }}</button>
                </div>
            </div>
        </div>
                        ...
                        @foreach (ThemeOption::constructFields($section['id']) as $field)
                            <div class="form-group mb-3 @if ($errors->has($field['attributes']['name'])) has-error @endif">
                                ...
                                @if (array_key_exists('helper', $field))
                                    <span class="help-block" style="color: red;">{!! BaseHelper::clean($field['helper']) !!}</span>
                                @endif
                            </div>
                            <hr>
                        @endforeach
